...
async def segment_image_with_prompts(
    image_path: str,
    points: str = "[]",     # JSON 字符串，例如 [[100, 150], [200, 250]]
    labels: str = "[]",     # JSON 字符串，例如 [1, 0]
    box: str = "[]",        # JSON 字符串，例如 [50, 50, 300, 300]
    ...
    参数:
        image_path (str): 图像路径
        points (str): JSON 字符串格式的点坐标 [[x,y], [x,y], ...]
        labels (str): JSON 字符串格式的点标签 [1,0,...] (1=前景,0=背景)
        box (str): JSON 字符串格式的矩形框 [x_min, y_min, x_max, y_max]
        multimask_output (bool): 是否返回多个候选掩膜
    ...
    # 解析提示参数
    point_coords = np.array(json.loads(points)) if points != "[]" else None
    point_labels = np.array(json.loads(labels)) if labels != "[]" else None
    box_coords = np.array(json.loads(box)) if box != "[]" else None
    ...
    return f"✅ 分割完成，掩膜已保存到: {output_path}"
